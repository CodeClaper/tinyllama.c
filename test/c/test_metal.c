/*
 * test_metal.c — Metal matmul backend unit tests (built only with
 * METAL=1, macOS only).
 *
 * Mirror of test_gpu.c against the Metal backend:
 *
 *   Part A — exact decode:  metal_dequant_batch() must reproduce
 *            gguf_dequant() bit-for-bit on the same raw bytes.
 *   Part B — matmul:  metal_matvec()/metal_matmat() (both trans
...
static u64 make_tensor(u32 type, u64 be, u64 bb,
    ...
    u64 bytes = n_blocks * bb + METAL_TEST_PAD;
    *buf = malloc(bytes);
...
static int check_dequant_exact(TensorInfo *ti) {
    ...
    for (u64 i = 0; i < n; i++) {
        ...
        if (a.u != b.u) {
            if (bad < 5)
                printf("    elem %llu: metal=0x%08x (%g) ref=0x%08x (%g)\n",
...
static void all_tests(void) {
    if (!metal_available()) {
        printf("test_metal requires a Metal device (macOS with Metal)\n");
        minunit_fail = 1;
        return;
    }
