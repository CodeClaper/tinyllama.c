...
static int check_dequant_exact(TensorInfo *ti, const u8 *buf) {
    ...
    if (gpu_dequant_batch(ti, buf, 0, n, out) != 0) {
...
static int check_matmul(TensorInfo *ti, const u8 *buf, u64 rows, u64 cols,
    ...
    int rc;
    if (batch == 1)
        rc = gpu_matvec(ti, buf, x, got, rows, cols, trans);
    else
        rc = gpu_matmat(ti, buf, x, got, batch, rows, cols, trans);
    if (rc != 0) {
...
MU_TEST(test_cache_hit) {
    ...
    mu_assert_int_eq(0, gpu_matvec(&ti, buf, x, y1, 64, 256, false));
    mu_assert_int_eq(0, gpu_matvec(&ti, buf, x, y2, 64, 256, false));
...
MU_TEST(test_bad_dims) {
    ...
    make_tensor(GGUF_TYPE_Q4_K, 256, 144, 64, 256, &buf, &ti);
    float x[256], y[64];
    mu_assert(gpu_matvec(&ti, buf, x, y, 64, 300, false) != 0,
              "oversized cols should fail");
    mu_assert(gpu_matvec(NULL, buf, x, y, 64, 256, false) != 0,
