...
int metal_matvec(TensorInfo *ti, const float *x, float *y, u64 rows, u64 cols, bool trans);

/* Y[b] = W @ X[b] for b in [0,batch).  X is [batch x cols], Y is
 * [batch x rows].  A single launch covers all batches. */
int metal_matmat(TensorInfo *ti, const float *X, float *Y, u64 batch, u64 rows, u64 cols, bool trans);
