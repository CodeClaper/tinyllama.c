/*
 * quants_metal.m — Metal batch dequantization backend (macOS only).
 *
 * Objective-C host side of the Metal GPU backend; the kernels live in
 * quants_metal.metal and are embedded here via quants_metal_msl.h
 * (generated by the Makefile), then compiled at first use with
 * newLibraryWithSource().
 *
 * The API mirrors gpu/quants_gpu.cu exactly: every wrapper falls back
...
#include "../def.h"
#include "quants_metal.h"
#include "quants_metal_msl.h" /* quants_metal_msl[] — embedded kernel source */
 ...
 * device buffer at first use (see quants_metal.metal for the layout). */
...
/* Params structs — layout must match the structs in quants_metal.metal. */
typedef struct {
    u32  type;
 ...
 * quants_metal.metal, which names the kernels matmul_nt_<T>/matmul_t_<T>. */
...
static int metal_ensure_ready(void) {
    ...
    if (!g_library) {
        ...
        NSError *err = nil;
        g_library = [g_device newLibraryWithSource:[NSString stringWithUTF8String:quants_metal_msl]
